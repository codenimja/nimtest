...
test "CLI version command":
  ...
  check exitCode == 0
  assertOutputContains(output, "1.0.0")  # Your app version
  assertOutputContains(output, "AppName") # Your app name

test "CLI init command creates structure":
...
suite "Integration Tests":
  ...
  test "complete workflow":
    ...
    # Step 1: Initialize
    let (initOut, initCode) = runCliCommandInDir(projectDir, "init")
    check initCode == 0
    assertDirExists(projectDir / "src")
    
    # Step 2: Create component
    let (createOut, createCode) = runCliCommandInDir(projectDir, "create button")
    check createCode == 0
